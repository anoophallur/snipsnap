<%@ page import="java.util.*,
  ...
  ** @version $Id: localization.jsp 1838 2005-11-07 12:07:05Z leo $
  --%>
    ...
    <td>
      ...
      <select size="1" name="app.timezone">
        ...
        <c:forEach items="${timezones}" var="timezone">
          <option value="<c:out value='${timezone.value.ID}'/>" <c:if test="${newconfig.timezone == timezone.value.ID}">selected="selected"</c:if>><c:out value="${timezone.value.ID}"/> | <fmt:message key="${timezone.key}"/></option>
        </c:forEach>
      </select>
    </td>
  </tr>
