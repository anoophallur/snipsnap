...
/**
 * Central class for snips. <p/> TODO: cUser, mUser, cTime, ... -> modified to
 * composite object
 * 
 * @author Stephan J. Schmidt
 * @version $Id: SnipImpl.java 1716 2004-07-16 14:33:35Z leo $
 */
public class SnipImpl implements Snip {
	...
	private void init() {
		if (null == children) {
			children = SnipSpaceFactory.getInstance().getChildren(
	...
	/**
	 * Returns true, when the snip is a weblog. Currently only test against
	 * 'start'. Should be extendet to test a "weblog"-label
	 * 
	...
	public Snip getCommentedSnip() {
		if (null != commentedName && !"".equals(commentedName)
				&& null == comment) {
	...
	/**
	 * Get a list of child snips, ordered by date with the newest one first
	 * 
	 * @return List of child snips
	 */
	public List getChildrenDateOrder() {
		return SnipSpaceFactory.getInstance().getChildrenDateOrder(
				(Snip) Aspects.getThis(), 10);
	}

	public List getChildrenModifiedOrder() {
		return SnipSpaceFactory.getInstance().getChildrenModifiedOrder(
				(Snip) Aspects.getThis(), 10);
	}

	/**
	 * Add a child snip. Sets the parent of the child to this snip and <b>stores</b>
	 * the child because of the new parent.
	 * 
	 * @param snip
	 *            Snip to add as child
	 */
	...
	/**
	 * Directly sets the parent snip, does not add the snip to the parent. This
	 * is needed for restoring from storage. Better solution wanted
	 * 
	 * @param parentSnip
	 *            new parent snip of this snip
	 */
	...
	public String getParentName() {
		return this.parentName != null ? parentName : resolveParentName();
	}

	private String resolveParentName() {
		String[] snips = this.getName().split("/");
		if (snips.length == 1) {
			return null;
		} else {
	...
	/**
	 * Return a short version of the name. Useful for vertical snip listings,
	 * where the snips should not be to long. End of snip name will be replaced
	 * with "..."
	 * 
	...
	/**
	 * Return an encoded version of the name, especially spaces replaced with
	 * "+"
	 * 
	...
	public String getAttachmentString() {
		...
		File fileStorePath = new File(Application.get().getConfiguration()
				.getFileStore(), "snips");
		while (it.hasNext()) {
			Attachment att = (Attachment) it.next();
			File file = new File(fileStorePath, att.getLocation());
			if (file.exists()) {
				tmp.append(SnipLink.createLink(SnipLink.getSpaceRoot() + "/"
						+ SnipLink.encode(name), att.getName(), att.getName()));
	...
	public String toXML() {
		...
		RenderEngineLabel reLabel = (RenderEngineLabel) getLabels().getLabel(
				"RenderEngine");
		RenderEngine engine = null;
		if (reLabel != null) {
			try {
				engine = (RenderEngine) container.getComponentInstance(Class
						.forName(reLabel.getValue()));
		...
		if (null == engine) {
			engine = (RenderEngine) container
					.getComponentInstance(Components.DEFAULT_ENGINE);
		}
	...
	public Snip copy(String newName) {
		...
		while (attsIt.hasNext()) {
			...
			Attachment att = newSnip.getAttachments().addAttachment(
					oldAtt.getName(), oldAtt.getContentType(),
					oldAtt.getSize(), oldAtt.getLocation());
			att.setDate(oldAtt.getDate());
			String location = att.getLocation();
			File attFile = new File(fileStorePath, location);
			if (attFile.exists()) {
				try {
					File newLocation = new File(newSnip.getName(), attFile
							.getName());
					File newAttFile = new File(fileStorePath, newLocation
							.getPath());
					newAttFile.getParentFile().mkdirs();
					BufferedOutputStream out = new BufferedOutputStream(
							new FileOutputStream(newAttFile));
					BufferedInputStream in = new BufferedInputStream(
							new FileInputStream(attFile));
					...
					Logger.warn("SnipImpl: unable to copy attachment: "
							+ attFile, e);
				}
			} else {
